...
class OrderDeliveredSubscriber {
  ...
  constructor({
    ...
    eventBusService.subscribe(
      ...
      async (data) => {
        console.log(
          "Template id from event handler: " +
            process.env.ORDER_DELIVERED_TEMPLATE
        );
        ...
        try {
          sendgridService.sendEmail({
            templateId: process.env.ORDER_DELIVERED_TEMPLATE,
            from: {
      ...
      {
        subscriberId: "order-delivered-subscriber",
      }
    );
  }

  orderPlacedData = async (order: Order) => {
...
export default OrderDeliveredSubscriber;
